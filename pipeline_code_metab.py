...
def colored_ICP(source, target):
    ...
    #sdraw_registration_result(source, target, current_transformation)
...
def process_file_list_comp_v2(file_id, dpath,tform):
    ...
    # TEMPORARY -- put this code back | pcd_tsdf.transform(tform)
...
rgb_model_path = '/home/vigir3d/Datasets/cattle_scans/maskrcnn_data/maskrcnn_oct_2024.pth'
...
def write_indiviudal_pcds(file_id, dpath, pcd):
...
def main(input_path, tform_path, colored_icp_path=None, write_path=None):
    ...
    if not success:
        ...
        # METAB
        # exclude head cameras
        file_ids = [11,12,15,19,18,20]  #- METAB  (10 cam)
        #file_ids = [11,12,14,16,17,18]  # - Regular setup

        # Generate a list of volumes using list comprehension
        start = time.time()
        
        # OLD -- uses process_file_list_comp_v2
        ...
        pcds = [process_file_list_comp_v1(file_id, input_path, tform) for file_id, tform in zip(file_ids,initial_tforms)]
        
        o3d.visualization.draw_geometries(pcds, "Transformed Clouds")
        # import sys
        # sys.exit()
        if colored_icp_path is not None : 
            colored_ICP_transforms =  load_calibration_colored_ICP(colored_icp_path)
            pcds = apply_colored_ICP_from_calibration(pcds, colored_ICP_transforms)
            ptsdf = apply_colored_ICP_from_calibration(ptsdf, colored_ICP_transforms)
        ...
        write_individual_clouds = True

        if write_individual_clouds :
            val = [write_indiviudal_pcds(file_id, input_path,ptsdf_single) for file_id, ptsdf_single in zip(file_ids,ptsdf)]
