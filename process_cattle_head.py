...
def load_calibration_initial(tform_path, num_transforms=3):
...
def perform_pairwise_alignment_head(pcds):
    ...
    result = combine_pcds([p0,p1,p2,p3])
    return result
# Begin Main
...
def main(input_path, tform_path, colored_icp_path=None, write_path=None):
    ...
    if not success:
        ...
        pcds_tformed_combined = combine_pcds(pcds_tformed)
        # Refine using colored_ICP
        #pcd_all = perform_pairwise_alignment_head(pcds_tformed)

        animal_id = os.path.basename(os.path.normpath(input_path))
        print(animal_id)
        if write_path is None:
            print("No write")
            save_results(animal_id, input_path, pcds_tformed_combined)
        else : 
            print("No write -- ")
            save_results(animal_id, write_path,pcds_tformed_combined)
