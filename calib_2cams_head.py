...
def detect_tag(img, k, tag_size):
    ...
    # Filter out tags with ID less than 4 in this case
...
def load_filter_pcds(data_path,mode):  # returns a list of  pcds
    ...
    for i in range(len(file_ids)):
        ...
        np.savetxt(os.path.join(data_path,fname_tform), transform)
        rgbdc = o3d.geometry.RGBDImage.create_from_color_and_depth(
        ...
        tag_poses[i] = transform
        tag_pose = np.dot(tag_poses[0], np.linalg.inv(tag_poses[i]))
        current_transform[i] = tag_pose
        pcds.append(copy.deepcopy(pcd).transform(tag_pose))
