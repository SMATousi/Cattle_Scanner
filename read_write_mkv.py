...
def process_file_list_comp_v2(file_id, dpath):
    ws = 2
    # Format filenames
    dpath = dpath.rstrip('/')
...
def main(input_path):
    ...
    if not success:
        ...
        pcds = [process_file_list_comp_v2(file_id, input_path) for file_id in file_ids]
        
        animal_id = os.path.basename(os.path.normpath(input_path))
        
        for i in range(len(pcds)):
            pcd_filename = os.path.join(input_path, f"{animal_id}_{file_ids[i]}.ply")
            o3d.io.write_point_cloud(pcd_filename, pcds[i]) 
